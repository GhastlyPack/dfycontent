 ...
 * Thesis: Visual learners convert better when they SEE the output.
 * Hero is a big video placeholder, supporting sections are video thumbnails.
 */
...
function VideoThumb({ ratio = '9/16', label, runtime, name, role }: { ratio?: string; label: string; runtime: string; name: string; role: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      >
        ...
        <div style={{ position: 'absolute', bottom: 12, left: 12, right: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, color: FG, opacity: 0.85 }}>{label}</div>
    ...
    <>
      ...
      <div style={{ background: BG, color: FG, minHeight: '100vh', fontFamily: BODY }}>
        <header style={{ padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Link href="/" style={{ fontFamily: DISPLAY, fontWeight: 700, fontSize: 17, letterSpacing: '-0.01em', color: FG }}>
            DFY Content
          </Link>
          <Link
            href="/apply"
        ...
        <section style={{ maxWidth: 1280, margin: '0 auto', padding: '40px 24px 64px' }}>
          ...
          <div style={{ marginTop: 32, display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <span style={{ alignSelf: 'center', fontSize: 13, color: MUTED }}>
        ...
        <section style={{ borderTop: `1px solid ${BORDER}`, padding: '64px 24px', background: PANEL }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
              <VideoThumb label="“DMs are full.”" runtime="0:42" name="Jordan M." role="Founder, B2B SaaS" />
              <VideoThumb label="“Friends thought I filmed it.”" runtime="0:31" name="Priya S." role="Consultant" />
              <VideoThumb label="“4k → 18k followers.”" runtime="0:55" name="Marcus T." role="Agency owner" />
              <VideoThumb label="“Two-hour setup. Forever paid off.”" runtime="0:38" name="Sasha K." role="Operator" />
            </div>
          </div>
        </section>
